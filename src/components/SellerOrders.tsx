...
export const SellerOrders: React.FC<{
  ...
  const advance = async (order: SellerOrder, status: SellerFulfillmentStatus) => {
    setUpdatingId(order.id);
    setError('');
    try {
      const fulfillment = await sellerOrderApi.updateFulfillment(order.id, status);
...
const OrderCard: React.FC<{
  order: SellerOrder;
  busy: boolean;
  onAdvance: (order: SellerOrder, status: SellerFulfillmentStatus) => void;
}> = ({ order, busy, onAdvance }) => {
  const created = order.createdAt ? new Date(order.createdAt).toLocaleString() : 'Time unavailable';
  const fulfillment = order.fulfillment || { status: 'NEW' as const, updatedAt: null, allowedNextStatuses: ['PROCESSING' as const] };
  ...
  return <article className="rounded-2xl border p-4 dark:border-neutral-700">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <div className="rounded-xl border p-3 dark:border-neutral-700">
        ...
        {next ? <button type="button" disabled={busy} onClick={() => onAdvance(order, next)} className="mt-3 rounded-xl bg-neutral-950 px-4 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-50 dark:bg-lime-400 dark:text-neutral-950">{busy ? 'Updating…' : actionLabel[next]}</button>
