  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 space-y-6">
      ...
      <section className={`rounded-3xl border p-5 md:p-6 ${cancelled ? 'border-red-300 bg-red-50 dark:bg-red-950/20 dark:border-red-900' : 'border-neutral-200 bg-white dark:bg-neutral-900 dark:border-neutral-800'}`}>
        ...
        {cancelled && <div className="mt-5 rounded-2xl border border-red-300 bg-white/70 dark:bg-neutral-950/40 p-4"><p className="text-xs font-black uppercase tracking-wide text-red-700 dark:text-red-300">Cancellation reason</p><p className="mt-1 font-semibold">{order.cancellationReason || 'No cancellation reason was recorded.'}</p>{order.cancelledAt && <p className="text-xs text-neutral-500 mt-2">Cancelled {dateTime(order.cancelledAt)}</p>}{order.cancellationRequest?.refundStatus === 'initiated' && <p className="mt-3 text-sm font-bold text-amber-700 dark:text-amber-300">Full refund initiated to your original payment method. Awaiting payment-provider confirmation.</p>}{order.cancellationRequest?.refundStatus === 'processed' && <p className="mt-3 text-sm font-bold text-green-700 dark:text-green-300">Full refund confirmed to your original payment method.</p>}{order.cancellationRequest?.refundStatus === 'failed' && <p className="mt-3 text-sm font-bold text-red-700 dark:text-red-300">Refund needs attention. Please contact Vibe4You support.</p>}</div>}
      </section>
