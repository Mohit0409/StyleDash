  ...
  return <div className="max-w-md mx-auto px-4 py-12">
    ...
    <div className="bg-white dark:bg-neutral-900 border dark:border-neutral-800 rounded-3xl p-7 space-y-4 shadow-sm">
      ...
      <form onSubmit={submit} className="space-y-4">
        {mode === 'register' && <>
          <input aria-label="Full name" required minLength={2} maxLength={80} autoComplete="name" value={name} onChange={event => setName(event.target.value)} placeholder="Full name" className="w-full p-3 rounded-xl border dark:bg-neutral-800" />
          <input aria-label="Phone number" minLength={10} maxLength={20} autoComplete="tel" value={phone} onChange={event => setPhone(event.target.value)} placeholder="Phone number" className="w-full p-3 rounded-xl border dark:bg-neutral-800" />
        </>}
        <input aria-label="Email address" required type="email" autoComplete="email" maxLength={254} value={email} onChange={event => setEmail(event.target.value)} placeholder="Email" className="w-full p-3 rounded-xl border dark:bg-neutral-800" />
