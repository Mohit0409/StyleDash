    ...
    def calculate_order(self, payload: dict[str, Any], now: datetime | None = None) -> dict[str, Any]:
        ...
        with self.store.lock:
            ...
            for item in items:
                ...
                product_gst_rate = product.get("gstRate")
                if product_gst_rate not in (None, ""):
                    trusted_item["gstRate"] = str(product_gst_rate)
                for key, value in (
        ...
        default_tax_rate = _money(self.settings["taxRate"], "tax")
        taxes = Decimal("0")
        discount_ratio = (coupon_discount / subtotal) if subtotal > 0 else Decimal("0")
        for item in trusted_items:
            item_rate_value = item.get("gstRate")
            item_rate = default_tax_rate if item_rate_value in (None, "") else (_money(item_rate_value, "GST rate") / Decimal("100"))
