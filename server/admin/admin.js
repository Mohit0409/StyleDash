...
byId('content').addEventListener('click', async event => {
  ...
  try {
    ...
    if(action==='settlement') {
      const settlementAction=button.dataset.value;
      let payload={action:settlementAction};
      if(settlementAction==='CONFIRM_COLLECTION'){
        const note=prompt('Collection confirmation note (optional):'); if(note===null)return; payload.note=note.trim()||null;
      }
      if(settlementAction==='RELEASE'){
        const note=prompt('Release note (optional):'); if(note===null)return; payload.note=note.trim()||null;
      }
      if(settlementAction==='MARK_SETTLED'){
        const payoutReference=prompt('External payout/reference ID (required):');
        if(!payoutReference||payoutReference.trim().length<2)return;
        const note=prompt('Settlement note (optional):'); if(note===null)return;
        payload.payoutReference=payoutReference.trim(); payload.note=note.trim()||null;
      }
      await api(`/api/admin/settlements/${encodeURIComponent(button.dataset.id)}`,{method:'PATCH',body:JSON.stringify(payload)});
    }
    if(action==='vendor-commission') {
      const raw=prompt('StyleDash commission percentage (whole number 0 to 100):',button.dataset.value||'');
      if(raw===null)return; const commissionPercent=Number(raw);
      if(!Number.isInteger(commissionPercent)||commissionPercent<0||commissionPercent>100)throw new Error('Commission must be a whole percentage from 0 to 100.');
      await api(`/api/admin/vendors/${encodeURIComponent(button.dataset.id)}/commission`,{method:'PATCH',body:JSON.stringify({commissionPercent})});
    }
...
async function loadTab(tab) {
  ...
  try {
    ...
    if(tab==='settlements') return renderSettlements((await api('/api/admin/settlements')).settlements);
...
function settlementAction(status){
  if(status==='AWAITING_COLLECTION')return ['CONFIRM_COLLECTION','Confirm COD collection'];
  if(status==='PENDING_CLEARANCE')return ['RELEASE','Release after clearance'];
  if(status==='ELIGIBLE')return ['MARK_SETTLED','Record external payout'];
  return null;
}
function moneyPaise(value){return value==null?'—':`₹${(Number(value)/100).toLocaleString('en-IN',{minimumFractionDigits:2,maximumFractionDigits:2})}`;}
function renderSettlements(items){
  const cards=items.map(item=>{const action=settlementAction(item.status);return `<article class="card"><h3>${escapeText(item.shopName)} · Order ${escapeText(item.orderId)}</h3><p class="muted">${escapeText(String(item.paymentMethod||'').toUpperCase())} · order ${escapeText(item.orderStatus||'unknown')} · payment ${escapeText(item.currentPaymentStatus||'unknown')}</p><div class="facts"><div class="fact"><small>Gross</small><strong>${moneyPaise(item.grossAmountPaise)}</strong></div><div class="fact"><small>Commission ${item.commissionPercent==null?'':`(${escapeText(item.commissionPercent)}%)`}</small><strong>${moneyPaise(item.commissionAmountPaise)}</strong></div><div class="fact"><small>Seller net</small><strong>${moneyPaise(item.netAmountPaise)}</strong></div><div class="fact"><small>Status</small><strong>${escapeText(String(item.status).replaceAll('_',' '))}</strong></div></div><p class="muted">Delivered ${escapeText(item.deliveredAt)} · clearance ${escapeText(item.clearanceUntil)}</p>${item.holdReason?`<p class="error">Hold: ${escapeText(item.holdReason)}</p>`:''}${item.cancellationStatus&&!['REJECTED','CANCELLED'].includes(item.cancellationStatus)?`<p class="error">Active cancellation request: ${escapeText(item.cancellationStatus)}</p>`:''}${item.payoutReference?`<p class="muted">External payout reference: <strong>${escapeText(item.payoutReference)}</strong></p>`:''}${action?`<div class="actions"><button class="success" data-action="settlement" data-id="${escapeText(item.id)}" data-value="${action[0]}">${action[1]}</button></div>`:''}</article>`;}).join('');
  byId('content').innerHTML=`<h2>Seller settlements</h2><p class="muted"><strong>No money is transferred by this page.</strong> It records COD collection, clearance eligibility, and payouts completed through an external/manual channel. Server-side guards re-check refunds, cancellations and payment state for every action.</p><div class="grid">${cards||'<p>No seller settlements yet.</p>'}</div>`;
}
...
function renderVendors(items){byId('content').innerHTML=`<h2>Shop applications</h2><p class="muted">Commission is private-admin controlled. Existing settlement snapshots keep their original percentage; a changed commission applies to future settlements and any delivery still waiting for commission configuration.</p><div class="grid">${items.map(item=>`<article class="card"><h3>${escapeText(item.shopName)}</h3><p>${escapeText(item.ownerName)} · ${escapeText(item.registeredEmail)} · ${escapeText(item.registeredMobile)}</p><p class="muted">${escapeText(item.address)}, ${escapeText(item.city)} ${escapeText(item.pincode)} — ${escapeText(item.description)}</p>${item.rejectionReason?`<p class="error">${escapeText(item.rejectionReason)}</p>`:''}<div class="facts"><div class="fact"><small>Status</small><strong>${escapeText(item.status)}</strong></div><div class="fact"><small>Commission</small><strong>${item.commissionPercent==null?'Not configured':`${escapeText(item.commissionPercent)}%`}</strong></div></div><div class="actions"><button data-action="vendor-commission" data-id="${escapeText(item.id)}" data-value="${escapeText(item.commissionPercent??'')}">Set commission</button>${applicationTransitions(item.status).map(status=>`<button class="${['REJECTED','SUSPENDED'].includes(status)?'danger':'success'}" data-action="vendor" data-id="${escapeText(item.id)}" data-value="${status}">${escapeText(status.replaceAll('_',' '))}</button>`).join('')}</div></article>`).join('')||'<p>No applications.</p>'}</div>`;}
