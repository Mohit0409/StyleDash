...
async function createStoreProduct(){
  ...
  const values=await formDialog('Add product for local store',[
    ...
    {name:'optionMode',label:'Product options',type:'select',required:true,value:'both',options:PRODUCT_OPTION_MODES,autoCategoryDefault:true,help:'Choose only the options customers actually need. Beauty, electronics, home and general items default to single stock; clothing and footwear default to colour + size.'},{name:'hsnCode',label:'HSN code (4-8 digits)',maxLength:8,help:'Use the HSN confirmed for this exact product.'},{name:'gstRate',label:'GST rate %',type:'number',min:0,max:40,step:'0.01',help:'Use the current GST rate confirmed for this HSN/product.'},
...
async function editStoreProduct(button){
  ...
  const values=await formDialog('Edit product details',[
    ...
    {name:'department',label:'Department',type:'select',required:true,value:PRODUCT_DEPARTMENTS.includes(item.department)?item.department:'unisex',options:PRODUCT_DEPARTMENTS.map(value=>({value,label:value}))},{name:'category',label:'Category',type:'select',required:true,value:PRODUCT_CATEGORIES.includes(item.category)?item.category:'Clothing & Fashion',options:PRODUCT_CATEGORIES.map(value=>({value,label:value}))},{name:'subcategory',label:'Subcategory (optional; inferred when clear)',value:item.subcategory||item.attributes?.subcategory||'',maxLength:100},{name:'hsnCode',label:'HSN code (4-8 digits)',value:item.attributes?.hsnCode||'',maxLength:8},{name:'gstRate',label:'GST rate %',type:'number',value:item.attributes?.gstRate||'',min:0,max:40,step:'0.01'},{name:'deliveryType',label:'Delivery schedule',type:'select',required:true,value:'normal',options:DELIVERY_OPTIONS},
  ...
  const colourVariants=await uploadColourVariantImages(values.colourVariants);
  const payload={name:values.name,description:values.description,brand:values.brand||undefined,department:values.department,category:values.category,subcategory:values.subcategory||undefined,deliveryType:values.deliveryType,pricePaise:Math.round(price*100),originalPricePaise:Math.round(original*100),colourVariants,hsnCode:values.hsnCode||undefined,gstRate:values.gstRate||undefined,attributes:{...(item.attributes||{}),optionMode:values.optionMode},tryAtHomeEnabled:values.tryAtHomeEnabled==='true',exchangeAvailable:values.exchangeAvailable==='true'};
